...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (!empty($username) && !empty($password) && !empty($first_name) && !empty($email) && !empty($number)) {
    
    // Insert into `admin` table
    $add_admin = "INSERT INTO `admin`(`username`, `password`, `email`) VALUES ('$username', '$password','$email')";
    if (mysqli_query($conn, $add_admin)) {
      // Retrieve the inserted admin_id
      $admin_id = mysqli_insert_id($conn);

      // Insert into `admin_data` table
      $add_admin_data = "INSERT INTO `admin_data`(`admin_id`, `firstname`, `lastname`, `email`, `mobile_num`) 
      VALUES ('$admin_id', '$first_name', '$last_name','$email','$number')";
      if (mysqli_query($conn, $add_admin_data)) {
        header("Location: ./adminlist.php");
        exit;
      } else {
        $message = "Error adding admin data: " . mysqli_error($conn);
      }
    } else {
      $message = "Error adding admin: " . mysqli_error($conn);
    }
  } else {
    $message = "Fill all required input fields!";
  }
}
?>
